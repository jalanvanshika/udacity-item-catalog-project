...
def newItem():
    ...
    if request.method == 'POST':
        ...
        return redirect(url_for('categoryMenu',
                                category_id=cat.id, loggedIn=loggedIn))
    else:
        categories = session.query(Category).all()
...
def editItem(item_id):
    ...
    if not login_session['email'] == user.email:
        print "You are not creater of this item. /
        Only creater can change edit/delete the item."
        return render_template(
    ...
    if request.method == 'POST':
        ...
        return render_template(
            'editItem.html',
            categories=categories, loggedIn=loggedIn, item=item)
